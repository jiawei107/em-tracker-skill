...
def send_push_notification(title: str, desp: str):
    """发送Server酱推送通知"""
    if SERVERCHAN_SENDKEY:
        try:
            from serverchan_sdk import sc_send
            response = sc_send(SERVERCHAN_SENDKEY, title, desp, {"tags": "论文追踪|通知"})
            print(f"  - [消息推送] Server酱通知已发出 (结果: {response})")
        except ImportError:
            print(C_YELLOW + "  - [消息推送失败] 未安装 serverchan-sdk，如需推送请运行: pip install serverchan-sdk")
        except Exception as e:
            print(C_RED + f"  - [消息推送失败] 发生异常: {e}")


def process_and_track_manuscripts(all_manuscripts: List[Dict[str, Any]], account: dict, data_dir: str = 'data'):
    """处理和追踪稿件状态"""
    ...
    username_dir = os.path.join(data_dir, account['username'])
    ...
    for manuscript in all_manuscripts:
        ...
        if not os.path.exists(history_file):
            ...
            if safe_write_excel(new_record, history_file):
                ...
                push_title = f"新稿件收录: {account['journal_short_name']}"
                push_desp = f"**期刊**: {account['journal_full_name']}\n\n**标题**: {title}\n\n**编号**: {ms_number}\n\n**初始状态**: {new_data['Status']} ({new_data['StatusDate']})"
                send_push_notification(push_title, push_desp)
        else:
            try:
                ...
                if (last_record['Status'] == new_data['Status'] and 
                    ...
                    # 每次查询依然推送，告知当前情况
                    push_title = f"查询完成(状态未变): {account['journal_short_name']}"
                    push_desp = f"**期刊**: {account['journal_full_name']}\n\n**标题**: {title}\n\n**编号**: {ms_number}\n\n**当前状态**: {new_data['Status']} ({new_data['StatusDate']})"
                    send_push_notification(push_title, push_desp)
                else:
                    ...
                    if safe_write_excel(updated_df, history_file):
                        ...
                        push_title = f"稿件状态更新: {account['journal_short_name']}"
                        push_desp = f"**期刊**: {account['journal_full_name']}\n\n**标题**: {title}\n\n**编号**: {ms_number}\n\n**旧状态**: {last_record['Status']} ({last_record['StatusDate']})\n\n**新状态**: {new_data['Status']} ({new_data['StatusDate']})"
                        send_push_notification(push_title, push_desp)
...
def run_single_query(account: dict, data_dir: str = 'data'):
    """执行单次查询"""
    ...
    if all_manuscripts_data is not None:
        process_and_track_manuscripts(all_manuscripts_data, account, data_dir)
    else:
...
def main():
    ...
    print("\n" + C_CYAN + "=" * 60)
    print(C_CYAN + C_BOLD + " Journal Manuscript Tracker (v31.1) - 自动遍历模式")
    print(C_CYAN + "=" * 60)

    # 获取有效账户数据
    valid_accounts = [acc for acc in ACCOUNTS if 'YOUR_USERNAME' not in acc.get('username', '').upper()]

    if not valid_accounts:
        print(C_YELLOW + "  -> config.py 中未配置任何有效账户。")
        return

    print(C_BLUE + f"\n发现 {len(valid_accounts)} 个有效账户配置，开始自动遍历查询...")
    
    # 逐个查询
    for i, account in enumerate(valid_accounts):
        print(C_CYAN + "\n" + "-" * 50)
        print(C_CYAN + C_BOLD + f"正在处理第 {i+1}/{len(valid_accounts)} 个账户: {account['journal_short_name']} - {account['username']}")
        print(C_CYAN + "-" * 50)
        run_single_query(account)
        
        # 避免请求过于频繁
        if i < len(valid_accounts) - 1:
            print(C_YELLOW + f"\n等待 {RETRY_DELAY_SECONDS} 秒后继续查询下一个账户...")
            time.sleep(RETRY_DELAY_SECONDS)
